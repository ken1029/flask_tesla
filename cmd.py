import json,os,urllib3,time

def RunCommand(BASE_URL, VEHICLE_ID, INPUT_CMD, PARAMETER_1, PARAMETER_2, TOKEN):
  # Run the appropriate command based on the value of INPUT_CMD
  if INPUT_CMD == "test_command":
    TestFunction(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "lock_doors":
    LockDoors(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "unlock_doors":
    UnlockDoors(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "honk_horn":
    HonkHorn(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "flash_lights":
    FlashLights(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "start_climate_control" or INPUT_CMD == "start_hvac":
    StartClimateControl(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "set_temps":
    SetTemps(BASE_URL, VEHICLE_ID, PARAMETER_1, TOKEN)
  elif INPUT_CMD == "start_defrost":
    StartDefrost(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "stop_defrost":
    StopDefrost(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "start_driver_seat_heater":
    StartDriverSeatHeater(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "start_front_passenger_seat_heater":
    StartFrontPassengerSeatHeater(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "start_sentry":
    StartSentry(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "stop_sentry":
    StopSentry(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "open_windows" or INPUT_CMD == "vent_windows":
    OpenWindows(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "close_windows":
    CloseWindows(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "actuate_frunk":
    ActuateFrunk(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "actuate_trunk":
    ActuateTrunk(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "set_charge_limit":
    SetChargeLimit(BASE_URL, VEHICLE_ID, PARAMETER_1, TOKEN)
    ...
    CloseChargePortDoor(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "set_charging_amps":
    SetChargingAmps(BASE_URL, VEHICLE_ID, PARAMETER_1, TOKEN)    
  elif INPUT_CMD == "enable_scheduled_charging":
    EnableScheduledCharging(BASE_URL, VEHICLE_ID, PARAMETER_1, TOKEN)
  elif INPUT_CMD == "disable_scheduled_charging":
    DisableScheduledCharging(BASE_URL, VEHICLE_ID, TOKEN)
  elif INPUT_CMD == "start_remote_drive":
    StartRemoteDrive(BASE_URL, VEHICLE_ID, PARAMETER_1, TOKEN)
  elif INPUT_CMD == "trigger_homelink":
    TriggerHomeLink(BASE_URL, VEHICLE_ID, TOKEN)
# Macro commands
    ...
    StopDefrost(BASE_URL, VEHICLE_ID, TOKEN)
    StopDriverSeatHeater(BASE_URL, VEHICLE_ID, TOKEN)
    StopFrontPassengerSeatHeater(BASE_URL, VEHICLE_ID, TOKEN)
    StopClimateControl(BASE_URL, VEHICLE_ID, TOKEN)
  else:
    print("ERROR: " + INPUT_CMD + " is not a recognized command")

# Function that is used for stack testing
def TestFunction(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  TESTVAR = "TESTVAR_VALUE"
  return(TESTVAR)

# Function that retrieves the vehicle's data and returns it 
def GetVehicleData(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
...
def GetVehicleLocation(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  VEHICLE_DATA = GetVehicleDriveData(BASE_URL, VEHICLE_ID, TOKEN)
...
def GetDriverSeatPosition(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  VEHICLE_DATA = GetVehicleData(BASE_URL, VEHICLE_ID, TOKEN)
...
def GetFrontPassengerSeatPosition(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DRIVER_SEAT_POSITION = GetDriverSeatPosition(BASE_URL, VEHICLE_ID, TOKEN)
...
def WakeVehicle(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'Content-Type': 'application/json',
  ...
  if HTTP_REQUEST_STATUS_CODE == 200:
    ...
    while GetVehicleState(BASE_URL, VEHICLE_ID, TOKEN) != "online":
      # Variables
      COUNTER = 0
      if COUNTER > 20:
...
def LockDoors(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'Content-Type': 'application/json',
...
def UnlockDoors(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'Content-Type': 'application/json',
...
def HonkHorn(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'Content-Type': 'application/json',
...
def FlashLights(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'User-Agent': 'None'
...
def StartClimateControl(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'User-Agent': 'None'
...
def StopClimateControl(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'User-Agent': 'None'
...
def SetTemps(BASE_URL, VEHICLE_ID, VEHICLE_TEMP, TOKEN):
  # Variables
  DATA = {
...
def StartDefrost(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'on': 1
  }
...
def StopDefrost(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'on': 0
  }
...
def SetSeatHeater(BASE_URL, VEHICLE_ID, SEAT_HEATER_POSITION, SEAT_HEATER_LEVEL, TOKEN):
  DATA = {
...
def StartDriverSeatHeater(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DRIVER_SEAT_POSITION = GetDriverSeatPosition(BASE_URL, VEHICLE_ID, TOKEN)
  SEAT_HEATER_LEVEL = 3
  SetSeatHeater(BASE_URL, VEHICLE_ID, DRIVER_SEAT_POSITION, SEAT_HEATER_LEVEL, TOKEN)

# Function that activates the front passenger's seat heater
def StartFrontPassengerSeatHeater(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DRIVER_SEAT_POSITION = GetDriverSeatPosition(BASE_URL, VEHICLE_ID, TOKEN)
  SEAT_HEATER_LEVEL = 3
  if DRIVER_SEAT_POSITION == 0:
  ...
  SetSeatHeater(BASE_URL, VEHICLE_ID, FRONT_PASSENGER_SEAT_POSITION, SEAT_HEATER_LEVEL, TOKEN)

# Function that deactivates the driver's seat heater
def StopDriverSeatHeater(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DRIVER_SEAT_POSITION = GetDriverSeatPosition(BASE_URL, VEHICLE_ID, TOKEN)
  SEAT_HEATER_LEVEL = 0
  SetSeatHeater(BASE_URL, VEHICLE_ID, DRIVER_SEAT_POSITION, SEAT_HEATER_LEVEL, TOKEN)
...
def StopFrontPassengerSeatHeater(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DRIVER_SEAT_POSITION = GetDriverSeatPosition(BASE_URL, VEHICLE_ID, TOKEN)
  SEAT_HEATER_LEVEL = 0
  if DRIVER_SEAT_POSITION == 0:
...
def StartSentry(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'on': 1
  }
...
def StopSentry(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'on': 0
  }
...
def OpenWindows(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'command': 'vent',
    'lat': 0,
...
def CloseWindows(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'command': 'close',
    'lat': 0,
...
def ActuateFrunk(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'which_trunk': 'front'
  }
...
def ActuateTrunk(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'which_trunk': "rear"
  }
...
def SetChargeLimit(BASE_URL, VEHICLE_ID, VEHICLE_CHARGE_LIMIT, TOKEN):
  # Variables
  DATA = {
...
def StartCharging(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'User-Agent': 'None'
...
# Function that starts charging the vehicle
def StopCharging(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'User-Agent': 'None'
...
def OpenChargePortDoor(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'Content-Type': 'application/json',
...
def CloseChargePortDoor(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  HEADERS = {
    'Authorization': "Bearer " + TOKEN,
    'Content-Type': 'application/json',
...
def SetChargingAmps(BASE_URL, VEHICLE_ID, VEHICLE_CHARGING_AMPS, TOKEN):
  # Variables
  DATA = {
...
def EnableScheduledCharging(BASE_URL, VEHICLE_ID, SCHEDULED_CHARGING_TIME, TOKEN):
  # Variables
  DATA = {
    'enable': 1,
...
# Function that enables scheduled charging
def DisableScheduledCharging(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  DATA = {
    'enable': 0
  }
...
def StartRemoteDrive(BASE_URL, VEHICLE_ID, PASSWORD, TOKEN):
  # Variables
  DATA = {
...
def TriggerHomeLink(BASE_URL, VEHICLE_ID, TOKEN):
  # Variables
  VEHICLE_DATA = GetVehicleLocation(BASE_URL, VEHICLE_ID, TOKEN)
